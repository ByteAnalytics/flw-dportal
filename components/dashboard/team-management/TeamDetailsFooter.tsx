"use client";

import React from "react";
import CustomButton from "@/components/ui/custom-button";

interface TeamFooterProps {
  onClose: () => void;
  onEdit: () => void;
}

const TeamDetailsFooter: React.FC<TeamFooterProps> = ({ onClose, onEdit }) => {
  return (
    <div className="flex items-center justify-between pt-6 mt-auto border-t border-[#F3F4F6]">
      <CustomButton
        title="Close"
        onClick={onClose}
        textClassName="!text-[13px] font-[600] text-[#374151]"
        className="rounded-[8px] border border-[#E5E7EB] bg-white hover:bg-gray-50 min-w-[80px] md:h-[43px] h-[40px]"
      />
      <CustomButton
        title="Edit Team"
        showIcon
        iconPosition="left"
        sideIcon={
          <svg
            width="15"
            height="15"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
          </svg>
        }
        onClick={onEdit}
        textClassName="!text-[13px] font-[700] text-white"
        className="rounded-[12px] border min-w-[120px] md:h-[43px] h-[40px]"
      />
    </div>
  );
};

export default TeamDetailsFooter;
